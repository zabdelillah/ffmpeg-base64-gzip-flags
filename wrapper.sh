...
do
  if [ ${KILL+x} ]; then
    ...
    if [[ ! "$i" == *.mp3 ]]; then
      ...
      ffmpeg -nostdin -progress /dev/stderr -i "/tmp/$i" -filter_complex "zoompan=z=1:d=300:s=1080x1910:fps=60,format=yuv420p" -f h264 -r 60 -t 5 "/tmp/$i.mp4" -y 2> >(sed 's/^/[INITCONVERT] /') &
    else
